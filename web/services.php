<!-- <service-section-start> -->
<section class="service3 pq-services bg-purple">
    <div class="container">
        <div class="col-lg-12 wow  fadeInDown animated" style="visibility: visible; animation-name: fadeInDown;">
            <div class="pq-section-title-style-1 text-center">
                <h5 class="pq-section-title txt-white">Our Services</h5>
            </div>
        </div>
        <div class="row">

            <div class="col-xl-3 col-lg-6 col-md-6 wow  fadeInUp animated"
                style="visibility: visible; animation-name: fadeInUp;">
                <div class="pq-icon-box pq-style-2 txt-center">
                    <div class="pq-icon">
                        <a href="pcod.php"><img src="images/icons/diseases.png" class="icon-pd"></a>
                    </div>
                    <div class="pq-icon-box-content mb-30">
                        <h6 class="service-txt">Disease Management </h6>
                        <p class="pq-icon-box-description">
                            We provide personalized, comprehensive disease management services to improve your health.
                            Whether you're managing a chronic condition or recovering from illness, our healthcare team
                            is here to support you at every step.
                        </p>
                        <div class="pq-button-container">
                            <a class="pq-button pq-btn-link" href="#">
                                <span class="pq-btn-text">Know More</span>
                            </a>
                        </div>
                        <!-- <div class="ser-btn d-flex justify-content-center"> <a href="pcos.php" class="
                        secondary-btn md-btn bott0n-border"> Know More</a>
                     </div> -->
                    </div>
                </div>
            </div>
            <div class="col-xl-3 col-lg-6 col-md-6 wow  fadeInUp animated"
                style="visibility: visible; animation-name: fadeInUp;">
                <div class="pq-icon-box pq-style-2 txt-center">
                    <div class="pq-icon">
                        <a href="weight-management.php"><img src="images/icons/body-weight-scale .png"
                                class="icon-pd"></a>
                    </div>
                    <div class="pq-icon-box-content">
                        <h6 class="service-txt">Weight Mangement </h6>
                        <p class="pq-icon-box-description">We offer personalized weight management programs to help you
                            lose weight, maintain a healthy lifestyle, or address weight-related health issues with
                            expert support.</p>
                        <div class="pq-button-container mb-30">
                            <a class="pq-button pq-btn-link" href="weight-management.php">
                                <span class="pq-btn-text">Know More</span>
                            </a>
                        </div>
                        <!-- <div class="ser-btn d-flex justify-content-center"> <a href="thyroid.php" class="
                           secondary-btn md-btn bott0n-border"> Know More</a>
                     </div> -->
                    </div>
                </div>
            </div>
            <div class="col-xl-3 col-lg-6 col-md-6 wow  fadeInUp animated"
                style="visibility: visible; animation-name: fadeInUp;">
                <div class="pq-icon-box pq-style-2 txt-center">
                    <div class="pq-icon">
                        <a href="diabetes.php"><img src="images/icons/stress-management.png" class="icon-pd"></a>
                    </div>
                    <div class="pq-icon-box-content">
                        <h6 class="service-txt">Stress Management </h6>
                        <p class="pq-icon-box-description">We offer expert stress management programs to help you regain
                            control, improve mental well-being, and enhance your quality of life, with tailored
                            solutions for work, personal, or everyday stress.</p>
                        <div class="pq-button-container mb-30">
                            <a class="pq-button pq-btn-link" href="stress-management.php">
                                <span class="pq-btn-text">Know More</span>
                            </a>
                        </div>
                        <!-- <div class="ser-btn d-flex justify-content-center"> <a href="diabetes.php" class="
                           secondary-btn md-btn bott0n-border"> Know More</a> -->
                    </div>
                </div>
            </div>
            <div class="col-xl-3 col-lg-6 col-md-6 wow  fadeInUp animated"
                style="visibility: visible; animation-name: fadeInUp;">
                <div class="pq-icon-box pq-style-2 txt-center">
                    <div class="pq-icon">
                        <a href="#products"><img src="images/icons/supplement.png" class="icon-pd"></a>
                    </div>
                    <div class="pq-icon-box-content">
                        <h6 class="service-txt">Wellness Supplement
                            Products </h6>
                        <p class="pq-icon-box-description">We provide premium wellness supplements to enhance energy,
                            boost immunity, and improve mental clarity, helping you achieve optimal health and vitality
                        </p>
                        <div class="pq-button-container mb-30">
                            <a class="pq-button pq-btn-link" href="#">
                                <span class="pq-btn-text">Know More</span>
                            </a>
                        </div>
                        <!-- <div class="ser-btn d-flex justify-content-center"> <a href="diabetes.php" class="
                           secondary-btn md-btn bott0n-border"> Know More</a>
                     </div> -->
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
<!-- <service-section-end> -->